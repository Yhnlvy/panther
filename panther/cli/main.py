    ...
    if ini_path:
        ...
        for t in target:
            for root, dirnames, filenames in os.walk(t):
                for filename in fnmatch.filter(filenames, '.panther'):
                    panther_files.append(os.path.join(root, filename))

        if len(panther_files) > 1:
            LOG.error('Multiple .panther files found - scan separately or '
                      'choose one with --ini\n\t%s', ', '.join(panther_files))
            sys.exit(2)

        elif len(panther_files) == 1:
            ini_file = panther_files[0]
            LOG.info('Found project level .panther file: %s', panther_files[0])
