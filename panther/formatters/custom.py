...
default template: {abspath}:{line}: {test_id}[panther]: {severity}: {msg}

:Example:

/usr/lib/python3.6/site-packages/openlp/core/utils/__init__.py: \
405: B310[panther]: MEDIUM: Audit url open for permitted schemes. \
...
def report(manager, fileobj, sev_level, conf_level, lines=-1, template=None):
    ...
    :param manager: the panther manager object
    ...
    if template is None:
        msg_template = "{abspath}:{line}: {test_id}[panther]: {severity}: {msg}"
